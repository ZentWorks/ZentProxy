...
async function zentloop() {
  ...
  $('#zentloop-form').onsubmit=async(e)=>{
    ...
    try{await api('/api/v1/integrations/zentloop',{method:'PUT',body:JSON.stringify({enabled:$('#zentloop-enabled').checked,forward_unknown_hosts:$('#zentloop-forward-unknown').checked,upstream:$('#zentloop-upstream').value,secret:$('#zentloop-secret').value,fallback:$('#zentloop-fallback').value,ip_lists,rules})});toast('ZentLoop integration saved');await zentloop();}
